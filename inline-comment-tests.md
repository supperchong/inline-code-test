...
引入行内测试还有助于降低 AI 在代码理解过程中的读取成本。AI 可以先分层读取函数名、函数描述和真实用例，在不展开函数实现的情况下获得相对准确的行为描述；只有在需要精确判断、排查问题或修改实现时，再进一步读取函数内容。

此外，该方案也有利于在 IDE 中进行快速测试和调试。不过在 AI Coding 时代，这一能力更适合作为面向人工编码的辅助收益，而不是本规范的主要目标。
...
Inline Test 还可以作为 IDE 插件的直接输入，用于提升人工编码时的测试和调试效率。
...
4. 选中某条测试用例后点击 `debug` 时，将该注释样例视为调试入口，对应到该测试用例的参数，并启动调试流程。
...
需要强调的是，这一方向主要服务于人工编码、人工排障和人工调试流程；相较于 AI 理解、索引和自动化测试，它属于次要但很实用的附加收益。
...
6. 按需接入索引、检索和摘要流程。
7. 如需增强人工编码体验，可在 IDE 插件中接入 `test` 和 `debug` 入口，将 Inline Test 转化为可执行的测试与调试动作。
